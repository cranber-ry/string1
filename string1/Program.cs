...
while (true)
{
    ...
    switch (selectedOption)
    {
        ...
        case "5":
            sentences = Regex.Split(text, @"(?<!\w\.\w)(?<![A-Z][a-z]\.)(?<=\.|\?|\!)\s");
            var sentancesWithoutCommas = sentences.Where(sentences => !sentences.Contains(",")).ToList();

            foreach (var sentense in sentancesWithoutCommas)
            {
